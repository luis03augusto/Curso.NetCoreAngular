...
let DateTimeFormatePipePipe = class DateTimeFormatePipePipe extends _angular_common__WEBPACK_IMPORTED_MODULE_2__["DatePipe"] {
    transform(value, args) {
        return super.transform(value, src_app_util_Constants__WEBPACK_IMPORTED_MODULE_3__["Constants"].DATE_TIME_FMT);
    }
};
...
let AuthInterceptor = class AuthInterceptor {
    ...
    intercept(req, next) {
        if (localStorage.getItem('token') != null) {
            ...
            return next.handle(cloneReq).pipe(Object(rxjs_internal_operators_tap__WEBPACK_IMPORTED_MODULE_3__["tap"])(succ => { }, err => {
                if (err.status === 401) {
                    this.route.navigate(['user/login']);
                }
            }));
...
let EventosComponent = class EventosComponent {
    constructor(eventoService, modalService, fb, toastr, localeService) {
...
let NavComponent = class NavComponent {
    constructor(authService, rooute, toast) {
...
let RegistrationComponent = class RegistrationComponent {
    constructor(authService, router, fb, toast) {
        this.authService = authService;
        this.router = router;
        this.fb = fb;
        this.toast = toast;
